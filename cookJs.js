 ...
 *               设计哲学：
 *               1. 我们不会阻止用户干愚事，因为那样也会妨碍用户做聪明的事儿
...
var cookJs = (function (global) {
   ...
   $runtime["paths"]["root"] = js[js.length-1].src.substring(0, js[js.length-1].src.lastIndexOf("/")+1) + "module/";
    ...
    return {
        ...
        config: function (module, configr) {
            ...
            if (typeof module === "object") {
                //cookJs框架设置接口
                configr = module;
                if (typeof configr.root === "string")
                    $runtime["paths"]["root"] = this.isEndWithSlash(configr.root)? configr.root : configr.root + "/";
                for (i in configr.paths) {
                    if (configr.paths.hasOwnProperty(i) && i != "root") {
                        $runtime["paths"][i] = this.isEndWithSlash(configr.paths[i])? 
                            configr.paths[i] : 
                            configr.paths[i] + "/";
                    }
                }
                for (i in configr.compat) {
        ...
        require: function (modules, callback) {
            ...
            //先格式化一下给定的模块名，目标为数组
            //并清理可能的额外空白字符
            modules = this.trim(this.isArray(modules) ? 
                    modules :
                ...
                if (allLoaded) {
                    ...
                    for (i = 0, len = modules.length; i < len; i++) {
                        for (j = 0, deps = $moduleMap[modules[i]].dependencies, depsLen = deps.length; j < depsLen; j++) {
                            if (self.inArray(deps[j], self.keys($moduleMap)) < 0)
                    ...
                    if (depsToRequire.length > 0) {
                        ...
                        self.isFunction(callback) && callback.apply($noop, arrangeEntity());
                    }
                }
            };
            ...
            //则直接执行回调函数
            cached && self.isFunction(callback) && callback(arrangeEntity());
        },
        /**
         * 扩展接口
         * 外部可以藉此接口扩展框架的基础函数库
        ...
        extend: function (ext) {
            var i;
            for (i in ext) {
                if (ext.hasOwnProperty(i) && i !== "require" && i !== "define" && i !== "ready")
                    this[i] = ext[i];
            }
        },
        /**
 ...
 * 2014/03/25    0.3-dev    兼容性改善，现在框架可在IE6+的浏览器中正常工作
 * 2014/03/28    0.3-dev    不再将cookJs注册到window，而是注册到引用框架的命名空间
 * 2014/04/07    0.3-dev    完善config方法
 */
